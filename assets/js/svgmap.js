(function($) {
    ...
    var mapSVG = function(elem, options) {
        if ($(elem).data('regions')) {
            options.regions = $(elem).data('regions');
        }
        var _data;
        this.methods = {
            destroy: function() {
            ...
            zoom: function(delta, exact) {
                ...
                if (!exact) {
                    ...
                    _data._zoomLevel += d;
                    if (_data._zoomLevel > _data.options.zoomLimit[1] || _data._zoomLevel < _data.options.zoomLimit[0])
                        return false;
            ...
            marksAdjustPosition: function(mark) {
                if (!mark && (!_data.RMarks || _data.RMarks.length < 1))
                    return false;
                var dx, dy;
            ...
            renderSVGCircle: function(item, parentTransform, parentStyle) {
                var x = $(item).attr('cx') || 0, y = $(item).attr('cy') || 0;
                r = $(item).attr('r') || 0;
